...
public class BCommentMgr {
	...
	public Vector<BCommentBean> getBComment(int num){
		...
		try {
			...
			sql = "select * from comm where num=? order by cnum desc";
			...
			while(rs.next()){
				...
				bean.setCommentt(rs.getString("commentt"));
	...
	public void insertBComment(BCommentBean bean){
		...
		try {
			con = pool.getConnection();
			sql = "insert into comm(name,commentt,regdate,num,cnum) "
					+ "values(?,?,SYSDATE,?,tmp_seq.NEXTVAL)";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, bean.getName());
			pstmt.setString(2, bean.getCommentt());
	...
	public int getBCommentCount(int num){
		...
		try {
			con = pool.getConnection();
			sql = "select count(num) from comm where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
			if(rs.next())
	...
	public void deleteBComment(int cnum){
		...
		try {
			con = pool.getConnection();
			sql = "delete from comm where cnum=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, cnum);
			pstmt.executeUpdate();
		} catch (Exception e) {
	...
	public void deleteAllBComment(int num){
		...
		try {
			con = pool.getConnection();
			sql = "delete from comm where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
		} catch (Exception e) {
